\part{Work on the KC705 FPGA firmware}
...
arithmetic blocks and Ethernet Medium Access Control (MAC) units, Block Random Access Memories (BRAMs), Digital Signal Processors (DSPs) and more.
\\
...
\subsection{Vivado}
\noindent In the MoVeIT project for the simulation, synthesis and implementation of the firmware it was used only Xilinx Vivado software.
\newline
...
This means that if a channel on one of the two FMC connector goes defective the user does not get any message or feedback.
For example, if \textit{ch00\_counter} is always 0, even if the test pulses (TP) are being sent to the chip, is up to the user to understand if the chip is bad or the FMC channel is damaged. This problem can be solved using an oscilloscope to probe the selected channel, however that's not a practical and fast solution.
