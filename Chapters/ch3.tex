\part{Work on the kc705\_tera10 FPGA firmware}
...
\noindent The Kintex7 KC705 board does not have a way to auto-check if all of the devices on the board are working properly.
This means that if a channel on one of the two FMC connector goes defective the user does not get any feedback.
For example, if \textit{ch00\_counter} is always 0, even if the test pulses (TP) are being sent to the chip, is up to the user to understand if the chip is bad or the FMC channel is damaged. This problem can be easily solved with the use of an oscilloscope used to probe the selected channel, however that's not a practical solution.
Thus the question is:
how to understand if all the I/O resources needed are working properly?
\\
This was my first contribution for the project.
In order to create a easy to use debug tool for the FMC connectors of the board I created a completely new firmware that sets all the FMC pins as an output.
With the use of an LVDS IP output buffer the pin pairs can be driven with a \textit{high}/\textit{low} logic signal. The user is able to change state with the use of a dip switch.
